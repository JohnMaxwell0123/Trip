...
> **当前生产版本**：`RELEASE v3.5.4 · 2026.09` ｜ **全球直达**：[trip-cts.pages.dev](https://trip-cts.pages.dev) ｜ **主分支 Commit**：`444f7b2`  
...
1. **长辈关怀大字模式 (Senior Care Mode · 拟在新会话中落地)**：为 50+ 团队成员深度定制一键大字关怀切换（字号放大 25%~35%、深沉高对比度抗大漠眩光、48px+ 防颠簸误触触控靶区、赶路如厕/酒店/门票优待高优降噪呈现、`localStorage` 状态持久化与防闪烁）；
2. **PWA 离线支持 (Service Worker)**：为青海湖及柴达木部分弱网路段增加离线地图切片与离线路书缓存；
3. **多日天气/风沙预警联动**：接入甘肃/青海沿途城市降水与大风降温实时天气 API；
4. **团队位置共享预留**：预留多人位置信标接口或微信小程序桥接；
5. **多旅行项目 Portal 门户页**：未来新增旅行计划时，在根目录上线精美自驾足迹卡片总览大厅。
