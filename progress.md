...
- **Layer 1（悬浮控制层）**：
  - 顶部 Header 悬浮岛（`rgba(20, 13, 7, 0.58)` + `blur(24px)`，仅在【行程路书】展示，切入地图与贴士自动隐去以释放视野）
  - 日程切换药丸横向滑动条（`rgba(255, 255, 255, 0.08)`，贴合视口顶端）
  - 右侧悬浮垂直玻璃 Dock（`✨`二级图钉开关、`📍`自驾轨迹开关、`🛰️`卫星图层切换，已去除冗余居中按钮）
  - 底部路线信息胶囊条（`rgba(30, 20, 10, 0.55)` + `blur(20px)`，内嵌【适应视野】按钮）
...
- **v3.0 (二次切入地图卡死与 Pixel NaN 致命异常根治)**：
  ...
  - **路书底部版本发布标识上线**：在行程路书列表最下方新增高质感液态玻璃版本号徽章，并固化为后续每次上线必更规程；
- **v3.1 (顶部 Header 场景隔离与悬浮控件精简)**：
  - **顶部 Header 悬浮岛场景隔离**：将顶部品牌标题仅保留于【📅 行程路书】页面；切入【🗺 路线地图】与【⚠️ 贴士备忘】后自动滑出隐藏，地图顶部天数药丸栏提升至距离视口顶端 10px，地图可视面积提升约 80px，自适应外边距同步优化；
  - **地图悬浮控件去重**：按用户决策，彻底移除右侧垂直 Dock 中功能完全重复的 `🎯` 按钮，保留底部统计卡片内的【适应视野】按钮，右侧保留 ✨探索、📍定位、🛰️卫星 三键经典排布；
  - **版本号递增**：行程路书底部徽章同步更新至 `RELEASE v3.1 · 2026.09`。
- **v3.2 (9.24 兰州水岸云上酒店深度落地与住宿状态体系)**：
  - **首晚大本营确立**：9月24日抵达兰州正式入住「水岸云上酒店（兰州中山桥店）」（兰州市城关区南滨河东路873号，经纬度 `[103.8178, 36.0668]`）；
  - **住宿状态视觉体系**：引入 `hotelStatus` 语义属性（`confirmed` / `tentative` / `home`），已订酒店呈现高质感翡翠绿渐变卡片、`✅ 已确认预订入住：` 标签、高德地图一键导航及深度入住指南直达按钮；其他天数（D1~D7）清晰标注为 `建议住宿标准（待定）：`，精准契合实际预订进展；
  - **D0/D1 动线闭环重构**：
    1. **D0 机场接机真实公路轨迹**：在 `DAY_LEGS` 挂载 D0 兰州中川机场 ➔ 水岸云上酒店（70km 机场高速与南滨河路），地图单日模式呈现完整接机轨迹与酒店定制主图钉；
    2. **二级探索知识库与图钉扩容**：知识库与 `DAY_DETAILS` 正式扩充至 34 个核心点位，新增 `d0_hotel`（水岸云上酒店一线黄河夜景、客房供暖加湿隔音、考斯特门前装卸车位指南及紧邻的大众巷老字号美食街夜步动线）；
    3. **D1 晨间无缝启程**：由于下榻处就在中山桥畔，晨起直接推窗观白塔山与中山铁桥晨曦，早餐品牛肉面后前往省博，彻底取消原行程在中午重复折返中山桥的冗余路程；
  - **性能与渲染零损耗**：坚持纯静态单文件架构，DOM 与 AMap 事件无冗余轮询与重排，版本号递增至 `RELEASE v3.2 · 2026.09`。
- **v3.3 (自驾考斯特五大三段式弹性时间熔断与降级预案体系落地)**：
  - **背景与调研实证**：结合马蜂窝、小红书最新西北大环线（兰州-张掖-鼎新-额济纳-嘉峪关-瓜州-敦煌-大柴旦-茶卡-青海湖-西宁）实际路况、考斯特10+人团队生理节律、西北日落时空规律（日落 19:00~19:35，暮光持续至 20:05，夜市营业至深夜）进行全量复盘，提炼 5 个核心关键点；
  - **三段式弹性灰度检视架构 (3-Zone Elastic System)**：
    - 摒弃死板非黑即白的硬性分割，设计「🟢 主线方案 A ｜ 🟡 弹性缓冲期 ｜ 🔴 熔断降级 Fallback」三色温润折叠卡片（`time-cutoff-box`）；
    - 完美融入苹果液态玻璃视觉体系（毛玻璃磨砂、微发光边框、折叠/展开顺畅手势、移动端自适应）；
  - **五大关键决策点精准集成**：
    1. **Day 1 乌鞘岭长城公路观景决策**：≤ 13:15 走计划 A 盘山登长城；13:15~13:35 弹性视能见度；> 13:35 降级进安远服务区远眺，19:40 天黑前安抵张掖；
    2. **Day 3 额济纳黑城古遗址 vs 怪树林落日占位决策**：里程同步校准为往返约 400km/车程 4.5h；≤ 14:30 游黑城全量；14:30~15:15 压缩黑城为快线打卡；> 15:15 降级直奔怪树林抢占大漠残阳机位；
    3. **Day 4 嘉峪关三联景取舍与瓜州落日调度**：里程同步校准为真实约 560km/车程 6.5h；≤ 12:30 加游悬壁长城（舍第一墩）；12:30~13:15 举手表决；> 13:15 降级直奔瓜州大漠雕塑看落日与敦煌夜市；
    4. **Day 6 大柴旦翡翠湖大漫游与穿越节奏决策**：≤ 14:40 翡翠湖深度环游+服务区惬意休整；14:40~15:15 高速巡航+精简如厕；> 15:15 降级全线紧锁 G3011 主线高速，19:55 暮光褪去前安抵茶卡；
    5. **Day 7 国庆首日青海湖二郎剑避堵与亲水决策**：路况全绿且 ≤ 12:45 进二郎剑码头；延误 15~25min 表决；严重拥堵 > 30min 或 > 13:30 降级舍弃大门排队，转投 G109 湖南线观景台与湖滨牧道亲水免票免堵，反超大车流顺抵西宁庆功；
    6. **返程保护策略**：根据用户明确要求，Day 8 回程机票暂未出票，不设置返程送机熔断点，保留最大弹性；
  - **知识库同步与版本发布**：
...
1. **立即在此 `progress.md` 对应的章节追加技术记录与决策，并同步递增修改 `trip-map/index.html` 行程路书底部的版本号标签（如 `v3.1`, `v3.2`...）**；
