...
- **v3.0 (二次切入地图卡死与 Pixel NaN 致命异常根治)**：
  ...
  - **路书底部版本发布标识上线**：在行程路书列表最下方新增高质感液态玻璃版本号徽章（`RELEASE v3.0 · 2026.09`），并固化为后续每次上线必更规程；
  - **地图悬浮控件重叠探查**：确认右下角悬浮 Dock 第 4 按钮 `🎯`（居中全景）与底部统计面板内的文字按钮【适应视野】存在 100% 功能逻辑重复，建议精简面板内文字按钮，使路线信息展示更舒展。
